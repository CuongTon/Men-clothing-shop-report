...
if __name__ == '__main__':
    ...
    collection = 'KraftVN_airflow' #changelater
    current_KraftVN = retrieve_data_from_MongoDB(spark, database,collection)

    # find a new change data
    update_KraftVN = update_new_data(latest_KraftVN, current_KraftVN)

    # load data to MongoDB
    if update_KraftVN:
        # summary SCD
        total_input_items = update_KraftVN.count()
        expired_items = update_KraftVN.where("_id is not null").count()
        updated_items = update_KraftVN.where("_id is null").count()
        new_items = updated_items - expired_items

        load_data_to_MongoDB(update_KraftVN, 'append', database, collection)
