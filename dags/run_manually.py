...
def _update_current_date():
    current_date = date.today() - timedelta(days=generall_setting.delay_time_for_first_day_run_ETL)
    server = MongoClient("mongodb://192.168.1.20:27017")
    db = server[generall_setting.Mongo_Database]
    collection = db['time_dim']
    if collection.find_one({'date': f'{current_date}'}):
        with open('/home/cuongton/airflow/delay_time.json', 'r+') as read_file:
            text = read_file.read()
            dct = json.loads(text)
            dct['delay_time_for_rerun_S3'] =  dct['delay_time_for_rerun_S3'] - 1
            dct['delay_time_for_first_day_run_ETL'] = dct['delay_time_for_first_day_run_ETL'] - 1
        with open('/home/cuongton/airflow/delay_time.json', 'w') as write_file:
            json.dump(dct, write_file, indent=2)
    else:
        raise(KeyError)
...
with DAG(
    ...
    update_current_date = PythonOperator (
        task_id = 'update_current_date',
        python_callable=_update_current_date
    )
    ...
    trigger_same_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id='update_manually', # replace with your DAG id
        trigger_rule='none_failed'  
    )

    # Work flow
    start >> merge_and_put_S3 >> is_the_first_time >> [initial_load, incremental_load] >> daily_sale_data_mart >> update_current_date >> end >> trigger_same_dag